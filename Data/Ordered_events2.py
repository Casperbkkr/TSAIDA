...
for i in range(n_experiments):
    ...
    for k in range(n_events):
        ...
        if np.sum(used[start:start + max_length]) > 0:
            ...
            for d in range(0, dim):
                length = r1[:,d][0]
                lent = (event[start:start+length, d]).shape[0]
                sine_part = sines[:lent, d]
    ...
    for k in range(n_anomalous_events):
        ...
        B[:,:] = 5 #B[0,0]
        period = 2*np.pi/B
        cosines = anom_direction*amp*np.sin(x3 * B)
        f = period / dt
        ...
        if start + length[0,0] >= G2.shape[0] - length[0,0]:
            G2[start:, :] = cosine_part[:(G2.shape[0] - start)]
        else:
